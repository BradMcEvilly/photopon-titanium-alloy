...
class Item extends Base_Model
{
	...
	function get_all_by_cat($cat_id, $limit = false, $offset = false)
	{
		$this->db->from($this->table_name);
	...
	function count_all_by($conditions=array())
	{
		...
		if (isset($conditions['searchterm']) && trim($conditions['searchterm']) != "") {
			$this->db->where("(
										...
										price LIKE '%".$conditions['searchterm']."%'
									)", NULL, FALSE);
		}
		return $this->db->count_all_results();
	}
	...
	function get_all_by($conditions=array(),$limit=false,$offset=false)
	{
		...
		if (isset($conditions['searchterm']) && trim($conditions['searchterm']) != "") {
			$this->db->where("(
										name LIKE '%".$conditions['searchterm']."%' OR 
										description LIKE '%".$conditions['searchterm']."%' OR 
										coordinate LIKE '%".$conditions['searchterm']."%'
									 )", NULL, FALSE);
		}
	...
	{
		$query = $this->db->query('SELECT count( appuser_id ) as cnt, item_id
		FROM `bc_likes`
		GROUP BY item_id 
