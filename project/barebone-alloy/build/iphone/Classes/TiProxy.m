...
//Rare exceptions to indicate a bug in the _photopon code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_photopon = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_photopon]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.photopon.module"
